...
def __render_enhanced_md__(metadata: Path, **kwargs):
    ...
    if len(diff_items) != len(metadata.corrections_explanations):
        logger.error(
            f"Length of list of corrections  does not match the length of the list of explanations. {len(diff_items)} != {len(metadata.corrections_explanations)}"
        )
        exit(1)

    ss_a.seek(0)
    render_data = asdict(metadata).copy()
    render_data["text"] = api.strikethrough_errors(ss_a, diff_items)
    print(render_enhanced_md(ss_a, diff_items, metadata_obj))


def render_enhanced_md_parser(subparser_action):
