...
def get_plain_text_from_html(string_stream: TextIO) -> str:
    """
...
def strikethrough_errors(markdown_stream: TextIO, diff_items: List[DiffItem]):
    """
    Rewrite content of markdown stream by applying a list of diff items.

    The original content is strikethrough and it is followed by the new
    content.
    """
    ...
    for index, item in enumerate(diff_items, 1):
        ...
        if item.operation == "insert":
            item.new_value = f"{item.new_value}{footnote_index} "
        elif item.operation == "delete":
            item.new_value = (
                f"~~{item.original_value}~~ {item.new_value}{footnote_index}"
            )
        ...
        s, e = utils.apply_diff(mv, item, start)
        start = e
