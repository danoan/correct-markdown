...
MDE=python scripts/md-enhancer/md-enhancer.py
...
VENV_FOLDER=${OUTPUT_FOLDER}/venv
...
${VENV_FOLDER}:
	scripts/create-venv/create-venv.sh ${OUTPUT_FOLDER}

######################################
# Generate corrected version
######################################

${OUTPUT_FOLDER}/pin_original.json: | ${OUTPUT_FOLDER}
	jq --null-input --rawfile message ${MARKDOWN_FILE} '{"message":$$message}' > $@
...
${OUTPUT_FOLDER}/render-data.json: ${OUTPUT_FOLDER}/pin_original.json ${OUTPUT_FOLDER}/pin_correct.json ${OUTPUT_FOLDER}/explanation.json ${OUTPUT_FOLDER}/summarize.json ${OUTPUT_FOLDER}/word-definition.json
	jq -n --slurpfile F1 $< --slurpfile F2 $(word 2, $^) --slurpfile F3 $(word 3, $^) --slurpfile F4 $(word 4, $^) --slurpfile F5 $(word 5, $^) '{"Title":"${TITLE}", "Original":$$F1[0].message, "Corrected": $$F2[0].message, "CorrectionsExplanations": $$F3[0], "Summary":$$F4[0].message,"WordsDefinitions":$$F5[0]}' > $@
